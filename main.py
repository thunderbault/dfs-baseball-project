...
class Player:
    ...
    def __init__(self, name, player_id, handedness, lineup_spot, team, dksalary, home, opposing_team, years = ['2015', '2016', '2017', '2018']):
        self.name = name
...
class Batter(Player):
    ...
    def updateSplits(self):
        if self.player_id:
            with sqlite3.connect('players.sqlite') as conn:
                ...
                cur.execute('''SELECT * FROM 'Players' WHERE player_name=?''', (self.name,))
                existing_data = cur.fetchall()
                if existing_data:
                    update= '''UPDATE 'Players' '''
                    set =  '''SET player_id = ? WHERE player_name = ?'''
                    cur.execute(update+set, (self.player_id, self.name))
                else:
                    cur.execute('''INSERT INTO 'Players' (player_name,
                            player_id)
                            VALUES ( ?, ?)''', (self.name, self.player_id))
                conn.commit()
        for year in self.years:
            data_tables = []
            data_tables = brscraper.BRScraper().brscraper_in_comments(self.createURL(year, "splits_url"), table_ids = self.splits_tables)
            for data_table in data_tables:
                for table_row in data_table[0][data_table[1]]:
                    ...
                    if data_table[1] == "total":
                        ...
                        if not self.opposing_pitcher:
                            continue
                        elif self.opposing_pitcher.handedness == "R":
    ...
    def updateBVP(self):
        ...
        data_tables = []
        data_tables.append([brscraper.BRScraper().parse_tables(self.createURL(2017, "bvp_url"), table_ids = "batter_vs_pitcher_1"), "batter_vs_pitcher_1"])
        ...
        for table_row in data_tables[0][0][data_tables[0][1]]:
            ...
            if sql_table_name and table_row:
                with sqlite3.connect('2017' + str(self) + '.sqlite') as conn:
                    cur = conn.cursor()
                    cur.execute("""CREATE TABLE IF NOT EXISTS '%s'
                        (player_name TEXT UNIQUE, 
                        PA INTEGER, H INTEGER, DOUBLE INTEGER, TRIPLE INTEGER, HR INTEGER, 
    ...
    def getStats(self, year, sql_table):
        with sqlite3.connect('%s'%year + str(self) + '.sqlite') as conn:
            ...
            try:
                ...
                self.stats_from_database = cur.fetchall()
            except:
                self.stats_from_database = None
            


    def calcDKScore(self, year, sql_table, ballpark_data, lineup_factor):
        self.getStats(year, sql_table)
        if not self.stats_from_database:
            return None
        if self.stats_from_database[0][0] < 10:
            return None
        ...
        if sql_table == "{}bvp".format(year):
            try:
                self.dkscore += (3.0*float(ballpark_data[self.name]["Singles"])*float(self.stats_from_database[0][1]-self.stats_from_database[0][2]-self.stats_from_database[0][3]-self.stats_from_database[0][4])
                    +5.0*float(ballpark_data[self.name]["Doubles"])*float(self.stats_from_database[0][2])
                    +8.0*float(ballpark_data[self.name]["Triples"])*float(self.stats_from_database[0][3])
                    +10.0*float(ballpark_data[self.name]["HR"])*float(self.stats_from_database[0][4])
                    +2.0*float(self.stats_from_database[0][5])
                    +2.0*float(self.stats_from_database[0][7]+self.stats_from_database[0][8]))/float(self.stats_from_database[0][0])*0.10*float(lineup_factor[self.lineup_spot])
                self.dkdivisor += 0.10
            except:
                pass
            ...
            try:
                self.dkscore += (3.0*float(ballpark_data[self.name]["Singles"])*float(self.stats_from_database[0][1]-self.stats_from_database[0][2]-self.stats_from_database[0][3]-self.stats_from_database[0][4])
                    ...
                    +10.0*float(ballpark_data[self.name]["HR"])*float(self.stats_from_database[0][4])
                    +2.0*float(self.stats_from_database[0][5])
                    +2.0*float(self.stats_from_database[0][7]+self.stats_from_database[0][8]))/float(self.stats_from_database[0][0])*float(self.table_factor[sql_table])*float(lineup_factor[self.lineup_spot])
    ...
    def calcFDScore(self):
        ...
        if sql_table == "{}bvp".format(year):
            self.fdscore += float(3.0*ballpark_data[player_full_name]["Singles"]*float(stats_from_database[0][1]-stats_from_database[0][2]-stats_from_database[0][3]-stats_from_database[0][4])
                                        ...
                                        +3.0*float(stats_from_database[0][7]+stats_from_database[0][8]))/stats_from_database[0][0]*0.10*lineup_factor[self.lineup_spot]
            self.fddivisor += 0.10
            ...
            self.fdscore += float(3.0*ballpark_data[player_full_name]["Singles"]*float(stats_from_database[0][1]-stats_from_database[0][2]-stats_from_database[0][3]-stats_from_database[0][4])
                                        ...
                                        +6.0*stats_from_database[0][9])/stats_from_database[0][0]*self.table_factor[sql_table]*lineup_factor[self.lineup_spot]
            self.fddivisor += float(self.table_factor[sql_table])
...
class Pitcher(Player):
    ...
    def updateSplits(self):
        for year in self.years:
            data_tables = []
            data_tables = brscraper.BRScraper().brscraper_in_comments(self.createURL(year), table_ids = self.splits_tables)
            for data_table in data_tables:
                for table_row in data_table[0][data_table[1]]:
                    sql_table_name = None
    ...
    def calcDKScore(self, year, sql_table, ballpark_data, lineup_factor):
        if not self.stats_from_database:
            return None
        if self.stats_from_database[0][0] < 4:
            return None
        self.dkscore += float(2.25*self.stats_from_database[0][0]+2.0*self.stats_from_database[0][2]+4.0*self.stats_from_database[0][3]
            -2.0*self.stats_from_database[0][4]-0.6*self.stats_from_database[0][0]*self.stats_from_database[0][5])/self.stats_from_database[0][1]*self.table_factor[sql_table]
        self.dkdivisor += self.table_factor[sql_table]
...
def constructPlayerList():
    starters_url = 'http://www.baseballpress.com/lineups/2017-09-06'
